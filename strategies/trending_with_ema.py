    ...
    def evaluate(self, date, position, market):
        ...
        for symbol, qty in position.items():
            if symbol != '$':
                ...
                print self.indicators[symbol]["short"].value
                print self.indicators[symbol]["long"].value
                print self.indicators[symbol]["short"].value - self.indicators[symbol]["long"].value
                '''
                #print self.diff
                if self.diff[symbol][-2] < 0 and  self.diff[symbol][-1] > 0:
                    buyTriggers.append(symbol)
                    self.diff[symbol] = [self.diff[symbol][-1] ]
                elif sorted(self.diff[symbol])[-1] != self.diff[symbol][-1]:
                    #print self.diff[symbol]
                    sellTriggers.append(symbol)
        #print sellTriggers
        #print buyTriggers
