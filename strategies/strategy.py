...
class Strategy(object):
    ...
    def addSpecificIndicator(self, symbol, name, indicator):
        object.__setattr__(self, symbol+name, indicator)
        if self.h5file != None:
            try:
                symgroup = self.h5file.getNode(self.indicator_h5group._v_pathname, symbol, classname="Group")  
            except tables.NoSuchNodeError:
    ...
    def updateSpecificIndicator(self, symbol, name, date, value=(0.0, 0)):
        indicator = object.__getattribute__(self, symbol+name)
        indicator.update(value, date)
