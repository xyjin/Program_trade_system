...
class Cache:
    ...
    def __init__(self):
        key_exists = 0
        self.db = MySQLdb.connect(host="localhost",user="root",passwd="root",db="market")
        ...
        cursor.execute("SHOW INDEX IN history")
        for row in cursor.fetchall():
            if 'ticker' in row:
                key_exists = 1
        if key_exists == 0:
            cursor.execute("CREATE UNIQUE INDEX ticker ON history (symbol (8), date ASC)")
        cursor.close()

    def symbols(self):
        """Return a list of symbols that have been cached."""
        cursor = self.db.cursor()
        cursor.execute("select distinct symbol from history")
    ...
    def get(self, symbol, wantDate, priorDate=None):
        try:
            db = MySQLdb.connect(host="localhost",user="root",passwd="root",db="market")
            cursor = db.cursor()
        #except OperationalError:
            
        #try:
            #cursor = self.db.cursor()
            logging.info("get symbol %s date ", symbol)
            if priorDate == None:
                hist = cursor.execute("SELECT * FROM HISTORY WHERE (date='%s' and symbol='%s')" % (wantDate, symbol))
            else:
                hist = cursor.execute("SELECT * FROM HISTORY WHERE (date>='%s' and date<='%s' and symbol='%s')" % (priorDate, wantDate, symbol))
            info = cursor.fetchall()
        except _mysql_exceptions.OperationalError:
            print 'date base fail'
        finally:
            cursor.close()
            db.close()
        return [Quote.fromRow(x) for x in info]

    def init(self, symbol, wantDate, priorDate):
        d = priorDate
        while d <= wantDate:
            cursor = self.db.cursor()
            cursor.execute("INSERT INTO HISTORY VALUES (NULL, '%s', '%s', NULL, NULL, NULL, NULL, NULL, NULL)" % (symbol, d))
            cursor.close()
            d += 1
        self.db.commit()
    ...
    def put(self, quotes):
        for quote in quotes:
            cursor = self.db.cursor()
            #cursor.execute( "DELETE FROM HISTORY where ('symbol=%(symbol)s', 'date=%(date)s', 'open=%(open)s', %(close)f, %(low)f, %(high)f, %(volume)f, %(adjclose)f )" % quote.__dict__)
            cursor.execute("REPLACE INTO HISTORY VALUES (NULL, '%(symbol)s', '%(date)s', '%(open)s', %(close)f, %(low)f, %(high)f, %(volume)f, %(adjclose)f )" % quote.__dict__)
            cursor.close()
        self.db.commit()

    def purge(self, symbol):
        cursor = self.db.cursor()
        hist = cursor.execute("DELETE FROM HISTORY WHERE (symbol='%s')" % (symbol))
        cursor.close()
        self.db.commit()
...
class Market:
    ...
    def loadIndexes(self):
        ...
        data = xlrd.open_workbook('D:\Personal\OneDrive\SingleRAN\Trander\quant-master\utils\A.xlsx')
        stock_table = data.sheets()[0]
        for index in range(0,stock_table.nrows):
            code = stock_table.cell(index,0).value
            if code < '600000':
                symbol = "%s.sz" % code
            else:
                symbol = "%s.ss" % code
            indexes.update({symbol:{}})
    ...
    def fetchHistory(self):
        ...
        try:
            for index in self.indexes.values():
                ...
                #index.fetchHistory()
                t=threading.Thread(index.fetchHistory())
                threads.append(t)
            for t in threads:
                t.start()
            for t in threads:
...
class Index:
    ...
    def fetchFromYahoo(self):
        ...
        if PROXY_NEED == 1:
            proxyConfig = 'http://%s' % ('87.254.212.120:8080')
            f = urllib.urlopen(url, proxies={'http':proxyConfig})
        else:
    ...
    def fetchHistory(self):
        ...
        #print "fetch history index %s" % self.symbol
...
class Ticker:
    ...
    def getQuote(self):
        ...
        if PROXY_NEED == 1:
            proxyConfig = 'http://%s' % ('87.254.212.120:8080')
            lines = urllib.urlopen(url, proxies={'http':proxyConfig})
        else:
    ...
    def __getitem__(self, date):
        ...
        logging.info("%s(1): date=%s" % (self.symbol, date))
    ...
    def _fetch(self, wantDate, priorDate=None, checkDuplicates=True):
        ...
        if priorDate == None:
            logging.info("fetching %s to %s-%s-%s" % (
                        self.symbol,
                        year2, month2, day2))
            parms = [
        ...
        try:
            if PROXY_NEED == 1:
                proxyConfig = 'http://%s' % ('87.254.212.120:8080')
                resp = urllib.urlopen(url, proxies={'http':proxyConfig})
            else:
    ...
    def fetchHistory(self, start=None, end=None):
        ...
        """
        if start == None:
            #startDay = QuoteDate.fromYmd(2014, 9, 1)
            startDay = None
        else:
    ...
    def updateHistory(self, start=None, end=None):
        ...
        if start == None:
            ...
            cursor.execute("select MAX(date) from history where (symbol='%s')" % self.symbol)
...
if __name__ == '__main__':
    ...
    china.updateHistory()
    #china.fetchHistory()
    #print china.symbolIndex
    #print china.indexes
    #print china.tickersBySymbol
    '''
    buf = Cache()
    qut = buf.get('600004.ss', '20140930', '20140919')
    print qut[1].adjopen
    '''
